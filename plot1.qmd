...
# Merge dataset into GeoJSON structure
region_map = alt.Chart(states).mark_geoshape().transform_lookup(
    lookup='id',
    from_=alt.LookupData(region_data, key='id', fields=['region'])
).transform_lookup(
    lookup='region',
    from_=alt.LookupData(df, key='region', fields=['any_household_energy_insecurity', 'total_cap_dc_region'])
).encode(
    color=alt.Color(
...
chart = alt.Chart(df).mark_rect().encode(
    ...
    y=alt.Y('any_household_energy_insecurity:Q',
            title='Energy Insecurity (%)',
            scale=alt.Scale(domain=[0, 8])),
    color=alt.Color('facility_count_division:Q',
    ...
    tooltip=[
        ...
        alt.Tooltip('any_household_energy_insecurity:Q', 
                   title='Energy Insecurity (%)', 
                   format='.1f'),
        alt.Tooltip('facility_count_division:Q', 
                   title='Number of Facilities')
    ]
).properties(
    width=600,
    height=400,
    title='Energy Insecurity and Facility Count by Division'
)

# Add text to show facility count values
text = alt.Chart(df).mark_text(
    baseline='middle',
    fontSize=11
).encode(
    x='division:N',
    y='any_household_energy_insecurity:Q',
    text=alt.Text('facility_count_division:Q', format='.0f'),
    color=alt.condition(
        alt.datum.facility_count_division > 100,
        alt.value('white'),
        alt.value('black')
    )
)
